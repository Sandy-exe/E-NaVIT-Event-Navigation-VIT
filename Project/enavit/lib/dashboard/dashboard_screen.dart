          ...
          PopupMenuButton<String>(
            onSelected: (String result) async {
              if (result == 'Update Expense') {
                ...
                if (expenseData != null &&
                    expenseData['description']!.isNotEmpty &&
                    expenseData['expense']!.isNotEmpty) {
                  Services service = Services();
                  String response =
                      await service.updateExpense(expenseData, event);
                ...
                if (userInput != null && userInput.isNotEmpty) {
                  Services service = Services();
                  String response =
                      service.updateExpectedRevenue(userInput, event) as String;
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(response),
                    ),
                ...
                List<Map<dynamic, dynamic>> expenses =
                    await Services().getExpenses(event);
                showDialog<void>(
                  context: context,
                  builder: (BuildContext context) {
                    return AlertDialog(
