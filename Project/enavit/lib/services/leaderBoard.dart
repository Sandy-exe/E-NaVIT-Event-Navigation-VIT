...
class LeaderBoard {
 ...
 Future<List<Event>> getRankingEvents() async {
  try {
    ...
    docs.sort((a, b) {
      double revenueA = double.tryParse(a['revenue']) ?? 0.0;
      double revenueB = double.tryParse(b['revenue']) ?? 0.0;
      return revenueB.compareTo(revenueA); // Sort in descending order
    });
  ...
  Future<List<Club>> getRankingClubs() async {
    try {
      ...
      // Convert revenue to double and sort
      docs.sort((a, b) {
        double revenueA = double.tryParse(a['revenue'].toString()) ?? 0.0;
        double revenueB = double.tryParse(b['revenue'].toString()) ?? 0.0;
        return revenueB.compareTo(revenueA); // Sort in descending order
      });
