    ...
    try {
      if (currentUserData['role'] == 0 || currentUserData['role'] == 1) {
        ...
        statdata['totalrevenue'] =
            double.parse(event.fee) * event.participants.length;
        ...
        for (var element in allusers.docs) {
          if (allparticipants.contains(element.data()['userid']) &&
              element.data()['reg_no'] != null) {
            if (element.data()['reg_no'].substring(2, 5).toUpperCase() ==
                "BRS") {
              statdata['BRS']++;
            } else if (element.data()['reg_no'].substring(2, 5).toUpperCase() ==
                "BCE") {
              statdata['BCE']++;
            } else if (element.data()['reg_no'].substring(2, 5).toUpperCase() ==
                "AIML") {
