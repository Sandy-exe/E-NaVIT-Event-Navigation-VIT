import 'package:enavit_main/dashboard/components/card_info.dart';
import 'package:enavit_main/dashboard/constants.dart';
import 'package:enavit_main/dashboard/models/stats_models.dart';
import 'package:enavit_main/models/og_models.dart';
import 'package:enavit_main/services/stats_services.dart';
  ...
  Future<void> statDataRetreive(Event event) async {
    ...
    attendanceIssue = [
      ...
      SquareCardInfo(
        ...
        color: primaryColor,
        percentage: ((statData['issuesSolved'] as int).toDouble() == 0.0
                ? 1
                : 0 / (statData['totalIssues'] as int).toDouble()) *
            100,
      ),
    ];
    ...
    financeCard = [
      FinanceCardInfo(
        ...
        repercentage: ((statData['totalrevenue'] == 0.0
                ? 1
                : statData['totalrevenue'] /
                    double.parse(statData['expectedrevenue'] == 0.0
                        ? 1
                        : statData['expectedrevenue'])) *
            100),
        expercentage: ((statData['totalexpense'] /
